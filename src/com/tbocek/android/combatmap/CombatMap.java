...
public final class CombatMap extends SherlockActivity {
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		mTokenSelector
				.setOnClickTokenManagerListener(new View.OnClickListener() {
					...
					public void onClick(final View arg0) {
						...
						startActivity(new Intent(CombatMap.this,
								TokenManager.class));
					}
				});
		...
		mTokenCategorySelector.setLayoutParams(new FrameLayout.LayoutParams(
				...
				FrameLayout.LayoutParams.MATCH_PARENT));
		mTokenCategorySelector
				.setOnTagListActionListener(mOnTagListActionListener);
		...
		if (mTabManager != null) {
			mTabManager.addTab(getString(R.string.background),
					MODE_DRAW_BACKGROUND, true);
			mTabManager.addTab(getString(R.string.gm_notes),
					MODE_DRAW_GM_NOTES, true);
			mTabManager.addTab(getString(R.string.combat), MODE_TOKENS, false);
			mTabManager.addTab(getString(R.string.annotations),
		...
		mCombatView.getMultiSelect().setSelectionChangedListener(
	...
	private void setUndoRedoEnabled() {
		...
		if (mUndoMenuItem != null) {
			mUndoMenuItem.setEnabled(mCombatView.getUndoRedoTarget().canUndo());
			mUndoMenuItem.setIcon(mUndoMenuItem.isEnabled() ? R.drawable.undo
					: R.drawable.undo_greyscale);
		}
		if (mRedoMenuItem != null) {
			mRedoMenuItem.setEnabled(mCombatView.getUndoRedoTarget().canRedo());
			mRedoMenuItem.setIcon(mRedoMenuItem.isEnabled() ? R.drawable.redo
	...
	private void reloadPreferences() {
		...
		if (mTabManager != null) {
			mTabManager.pickTab(mSharedPreferences.getInt("manipulation_mode",
					MODE_DRAW_BACKGROUND));
		}
	...
	public boolean onOptionsItemSelected(final MenuItem item) {
		...
		case R.id.menu_clear_all:
			...
			if (mSharedPreferences.getBoolean("autosave", true)) {
				new MapSaver(mSharedPreferences.getString("filename", ""),
	...
	private void setManipulationMode(final int manipulationMode) {
		...
		case MODE_TOKENS:
			...
			mCombatView.setFogOfWarMode(mSharedPreferences.getBoolean(
					"fogofwar", true) ? FogOfWarMode.CLIP
	...
	private void loadModePreference() {
		...
		setManipulationMode(mSharedPreferences.getInt("manipulation_mode",
				MODE_DRAW_BACKGROUND));
	}
			...
			return new TextPromptDialog(this,
					new TextPromptDialog.OnTextConfirmedListener() {
						public void onTextConfirmed(final String text) {
							...
							if (!text.equals(mSharedPreferences.getString(
									"filename", ""))
								...
								new MapSaver(text, getApplicationContext())
										.run();
							}
						}
					...
					.setPositiveButton("Yes",
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
									...
									new MapSaver(mAttemptedMapName,
	...
	protected void onPrepareDialog(final int id, final Dialog dialog) {
		...
		case DIALOG_ID_DRAW_TEXT:
			...
			if (mEditedTextObject != null) {
				fd.populateFields(mEditedTextObject.getText(),
		...
		case DIALOG_ID_SAVE_NAME_CONFIRM:
			...
			ad.setMessage("There is already a map named \"" + mAttemptedMapName
					+ "\".  Save over it?");
			break;
		case DIALOG_ID_GRID_PROPERTIES:
	...
	private class SelectionChangedListener implements
		...
		public void selectionChanged() {
			...
			if (mMultiSelectActionMode != null && selected.size() > 0) {
				...
				mMultiSelectActionMode.setTitle(Integer.toString(numTokens)
	...
	class TokenDatabaseLoadTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			mTokenDatabase = TokenDatabase.getInstance(getApplicationContext());
