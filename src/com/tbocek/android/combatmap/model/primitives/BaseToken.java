...
public abstract class BaseToken {
	...
	public final void move(final float distanceX, final float distanceY) {
		setLocation(new PointF(getLocation().x - distanceX, getLocation().y
				- distanceY));
	}
	...
	public final void drawGhost(final Canvas c,
		...
		float radius = transformer.worldSpaceToScreenSpace(this.getSize()
				* TOKEN_SIZE_TWEAK / 2);
	...
	public void draw(Canvas c, float x, float y, float radius,
		...
		if (isBloodied()) {
			...
			drawImpl(c, x, y, radius, darkBackground, isManipulatable);
		}
	...
	public final BoundingRectangle getBoundingRectangle() {
		...
		r.updateBounds(new PointF(mLocation.x - mSize / 2, mLocation.y - mSize
				/ 2));
		r.updateBounds(new PointF(mLocation.x + mSize / 2, mLocation.y + mSize
				/ 2));
		return r;
