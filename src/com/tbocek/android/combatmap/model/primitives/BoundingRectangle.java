...
public final class BoundingRectangle {
	...
	public boolean contains(final PointF p) {
		return p.x >= mXMin && p.x <= mXMax && p.y >= mYMin && p.y <= mYMax;
	}
	 ...
	 */
	public boolean intersectsWithCircle(final PointF center, final float radius) {
		return center.x + radius >= mXMin && center.x - radius <= mXMax
				&& center.y + radius >= mYMin && center.y - radius <= mYMax;
	}
