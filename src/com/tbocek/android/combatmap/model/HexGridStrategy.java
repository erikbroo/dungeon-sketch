...
public final class HexGridStrategy extends GridDrawStrategy {
    ...
    private PointF getNearestHexCorner(float centroidX, float centroidY, float candidateX, float candidateY) {
    	...
    	for (float theta = 0; theta < 2 * Math.PI; theta += Math.PI / 3) {
    		...
    		float distance = Util.distance(candidateX, candidateY, cornerX, cornerY);
    ...
    public void drawGrid(final Canvas canvas,
        ...
        for (int j = -1; j <= numSquaresHorizontal + 1; ++j) {
            ...
            for (int i = -1; i <= numSquaresVertical + 1; ++i) {
                float y = i * h + offsetY
                    - (j - innerPointStartX) % 2 * .5f * h;
                canvas.drawLine(x, y, innerX, y + h / 2, paint);
                canvas.drawLine(
                    innerX, y + h / 2, x, y + h, paint);
                canvas.drawLine(
                    innerX, y + h / 2, innerX + l,
                    y + h / 2, paint);
            }
        }
    }
