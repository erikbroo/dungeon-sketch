...
/**
 * This class maintains a queue of tokens that need to be loaded.  Each entry
 * in the queue comes with a callback that is called when the token is loaded.
 * This callback is called in the UI thread so that UI elements can update with the
 * newly loaded token.
 * 
 * Token loads can be batched, so that the callback is only called when every
 * token in the batch is loaded.
 * 
 * Registering a token to load provides a handle that allows the load action to be
...
public class TokenLoadManager {
	...
	public class JobHandle {
		...
		private JobHandle(List<BaseToken> tokensToLoad, JobCallback callback, Handler uiThreadHandler) {
		...
		private void postResult() {
			mUiThreadHandler.post(new JobCallbackRunnableWrapper(mCallback, mTokensToLoad));
		}
	}
	...
	private class JobCallbackRunnableWrapper implements Runnable {
		...
		private JobCallbackRunnableWrapper(JobCallback callback, List<BaseToken> loadedTokens) {
			mCallback = callback;
	...
	public JobHandle startJob(List<BaseToken> tokensToLoad, JobCallback callback, Handler uiThreadHandler) {
		JobHandle handle = new JobHandle(tokensToLoad, callback, uiThreadHandler);
		try {
			mQueue.put(handle);
