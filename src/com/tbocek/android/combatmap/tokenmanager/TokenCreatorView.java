...
public final class TokenCreatorView extends View {
     ...
     * zooms.
     */
    private SimpleOnScaleGestureListener mOnScaleGesture = new SimpleOnScaleGestureListener() {
        @Override
    ...
    public TokenCreatorView(final Context context) {
        ...
        mScaleDetector = new ScaleGestureDetector(getContext(), mOnScaleGesture);
    ...
    public void setImage(final Drawable image) {
        ...
        if (this.getWidth() > 0) {
            ...
            mHasCircle = false; // Defer until the widget is sized into the
                                // layout.
        }
    ...
    public void drawHalfBrightnessImage(final Canvas canvas) {
        ...
        mCurrentImage.setColorFilter(new PorterDuffColorFilter(Color.GRAY,
    ...
    public void drawFullBrightnessCircle(final Canvas canvas) {
        ...
        Path p = new Path();
        p.addCircle(mCircleCenterX, mCircleCenterY, mCircleRadius,
                Path.Direction.CW);
        canvas.clipPath(p);
        mCurrentImage.draw(canvas);
        canvas.restore();
    ...
    public Bitmap getClippedBitmap() {
        ...
        int bitmapSquareSize = Math.min((int) squareSizeImageSpace,
                MAX_TOKEN_SIZE);

        Bitmap bitmap = Bitmap.createBitmap(bitmapSquareSize, bitmapSquareSize,
        ...
        PointF upperLeftImageSpace = new PointF(centerImageSpace.x
                - squareSizeImageSpace / 2, centerImageSpace.y
                - squareSizeImageSpace / 2);

        Rect clippingRect = new Rect((int) (-upperLeftImageSpace.x),
                (int) (-upperLeftImageSpace.y),
                (int) (-upperLeftImageSpace.x + mCurrentImage
                        .getIntrinsicWidth()),
                (int) (-upperLeftImageSpace.y + mCurrentImage
                        .getIntrinsicHeight()));
