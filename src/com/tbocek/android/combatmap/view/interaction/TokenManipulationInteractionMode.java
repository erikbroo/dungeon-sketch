...
public final class TokenManipulationInteractionMode extends
     ...
     */
    private ValueAnimator.AnimatorUpdateListener mTrashCanFadeListener = new ValueAnimator.AnimatorUpdateListener() {
    ...
    public boolean onScroll(final MotionEvent e1, final MotionEvent e2,
        ...
        if (mCurrentToken != null) {
            ...
            if (getView().shouldSnapToGrid()) {
                ...
                PointF nearestSnapPointWorldSpace = getData()
                        .getGrid()
                        .getNearestSnapPoint(
                                transformer
                ...
                debugSnapPoint = transformer
                        .worldSpaceToScreenSpace(nearestSnapPointWorldSpace);

                mCurrentToken
                        .setLocation(distanceToSnapPoint < GRID_SNAP_THRESHOLD ? nearestSnapPointWorldSpace
                                : transformer
                                        .screenSpaceToWorldSpace(currentPointScreenSpace));
            } else {
                mCurrentToken.setLocation(transformer
                        .screenSpaceToWorldSpace(currentPointScreenSpace));
            }
            mAboutToTrash = TRASH_CAN_RECT.contains((int) e2.getX(),
                    (int) e2.getY());
        } else {
    ...
    public void draw(final Canvas c) {
        if (mCurrentToken != null && mDown) {
            ...
            // from.
            mCurrentToken.drawGhost(c, getView().getGridSpaceTransformer(),
                    mOriginalLocation);
        }
        ...
        if (debugSnapPoint != null && mDown) {
            Paint p = new Paint();
            p.setColor(getData().getGrid().getColorScheme().getLineColor());
